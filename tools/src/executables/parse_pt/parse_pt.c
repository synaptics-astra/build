...
struct table_entry{
	...
	char write_protection[PART_NAME_MAX_LEN + 1];
};
...
int gen_emmc_part_list(const char *part_table_path, const char *part_list_path)
{
	...
	{
		fprintf(stderr, "too small partition table size: %ld\n", file_stat.st_size);
		ret = -1;
		goto END;
	}
...
int main(int argc, char **argv)
{
	...
	while(fgets(line_buffer, 512, fcfg))
	{
		...
		if (argc == NAND_PARAM_COUNT) {
			ret = sscanf(line_buffer, "%s%s%hhd%s%s%s",
					cfg_table[i].layout, cfg_table[i].name, &cfg_table[i].flash_plane, cfg_table[i].partition_type, str, cfg_table[i].write_protection);
			if (ret < 2) {
	...
	for(i=0;i<entries;i++)
	{
		...
		printf("part name %s, off %Ld, size %lld, plane %d, type %d, operation %s\n",
			cfg_table[i].name, cfg_table[i].start, cfg_table[i].size,
			cfg_table[i].flash_plane, cfg_table[i].flash_type, cfg_table[i].write_protection);
	}
	//generate files
	...
	for(i=0;i<entries;i++)
	{
		...
		if(strstr(cfg_table[i].write_protection, "normal"))
		{
			vt.write_protect_flag |= NORMAL_WP_MASK;
		}
		if(strstr(cfg_table[i].write_protection, "recovery"))
		{
			vt.write_protect_flag |= RECOVERY_WP_MASK;
		}

		if(strstr(cfg_table[i].write_protection, "all"))
		{
			vt.write_protect_flag |= NORMAL_WP_MASK | RECOVERY_WP_MASK;
		}
		vt.part2_version.major_version = 0;
		vt.part2_version.minor_version = 0;
